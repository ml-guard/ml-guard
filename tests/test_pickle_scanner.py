...
Covers:
  • Benign pickles yield no CRITICAL/HIGH (only optional MEDIUM for non-ML modules).
  • Each known RCE primitive is detected as CRITICAL.
  • PyTorch ZIP format is unpacked and scanned.
  • Corrupt pickle does not crash the scanner — emits MEDIUM parse-error.
"""
...
def test_benign_pickle_is_clean(benign_pickle_path: Path):
    ...
    # A benign pickle must produce no CRITICAL findings.
...
def test_detects_os_system(malicious_os_pickle_path: Path):
    ...
    # On Linux os.system serializes as posix.system; on Windows as nt.system.
    assert any(
...
def test_torch_zip_format(torch_zip_with_malicious_pickle: Path):
    ...
    # The malicious pickle lives inside the ZIP — we must still find it.
    assert _critical_count(findings) >= 1, [f.to_dict() for f in findings]
    # location must reference the ZIP member name
